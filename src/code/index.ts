...
const FetchJson = (query:string,search:string) => {
    fetch('public/json/ForkDatabase.json').then( (ReqProto:any) => {
...
const InnerData = (newData) => {
    ...
    for(let shell:number = 0 ; shell < newData.shells.length ; ++shell){
    ...
    const StyleIndex:string = newData.category
     ...
     */
     if(StyleIndex.includes('unknown') === true){ Color = "#888"}
     else if(StyleIndex.includes("Hydrogen") === true){ Color = "#f59"}
     else if (StyleIndex.includes("noble-gas") === true){ Color = "#50a"}
     else if(StyleIndex.includes("alkali-metal") === true){Color = "#12f" }
    else if (StyleIndex.includes("alkaline-earth-metal") === true){ Color = "#5af"}
    else if ( StyleIndex.includes("metalloid") === true){Color = "#a02"}
    else if (StyleIndex.includes("nonmetal") === true){Color = "#295"}
    else if (StyleIndex.includes("post-transition-metal") === true){Color = "#a5a"}
    else if (StyleIndex.includes("transition-metal") === true){Color = "#f54"}
    else if (StyleIndex.includes("lanthanide") === true){Color = "#fa5"}
    else if (StyleIndex.includes("actinide") === true){Color = "#f5f"}

    console.log(Color)

    const FooterDinamicStyle = document.querySelector("#FooterDinamicStyle") as HTMLStyleElement
